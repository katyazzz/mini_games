...
function determineWinner() {
    ...
    const movesText = `Игрок 1 выбрал: ${translateMove(player1Move)}. ${gameMode === 'friend' ? 'Игрок 2 выбрал' : 'Робот выбрал'}: ${translateMove(player2Move)}.`;
    ...
    if (player1Move === player2Move) {
        ...
        if (gameMode === 'friend') {
            ...
            resultText.textContent = `Вы выиграли! ${translateMove(player1Move)} побеждает ${translateMove(player2Move)}.`;
        }
    } else {
        if (gameMode === 'friend') {
            resultText.textContent = `Игрок 2 выиграл! ${translateMove(player2Move)} побеждает ${translateMove(player1Move)}.`;
        } else {
            resultText.textContent = `Робот выиграл! ${translateMove(player2Move)} побеждает ${translateMove(player1Move)}.`;
        }
    }
}
